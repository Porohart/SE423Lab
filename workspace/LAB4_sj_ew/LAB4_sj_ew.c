...
// sjew: custom functions
void setDACA(float volt);

// sjew: cus variables
float adcd1result_volts = 0;
float adcd0result_volts = 0;
uint32_t ADCD0_count = 0;
...
//    init_serialSCID(&SerialD,115200);
    ...
    // sjew: second ADC trigger because microphone is sampled at a 0.1ms rate
    ...
    EPwm7Regs.ETPS.bit.SOCAPRD = 0x1; // Generate pulse on 1st event (“pulse” is the same as “trigger”)
...
void setDACA(float dacouta0) {
    int16_t DACOutInt = 0;
    // sjew: convert voltage value to a digital value, constrain to range and then set the digital value to the DAC to output
    DACOutInt = dacouta0 * 4095.0 / 3.0; // perform scaling of 0 – almost 3.0V to 0 – 4095
...
__interrupt void ADCD_ISR (void)
{
    // sjew: get the adcd 0 and 1 values and convert to volts
    int16_t adcd0result = AdcdResultRegs.ADCRESULT0;
    int16_t adcd1result = AdcdResultRegs.ADCRESULT1;

    // Here covert ADCIND0 to volts
    adcd0result_volts = adcd0result * 3.0 / 4095.0;
    adcd1result_volts = adcd1result * 3.0 / 4095.0;

    // Here covert ADCIND0, ADCIND1 to volts
    // sjew: update the array to represent the 20 most recent values including the current value
    float prev = adcd0result_volts;
    for(int i = 0; i < 22; i++) {
        float tmp = xk_arr[i];
        xk_arr[i] = prev;
        prev = tmp;
    ...
    yk = 0;
    // populate the yk value by dot product of coefficients and 22 most recent values
    for(int i = 0; i < 22; i++) {
        yk += b[i]*xk_arr[i];
    }
    // sjew: set DACA
...
__interrupt void ADCB_ISR (void) {
    // sjew: get the adc result from the register and convert it to a voltage value
    int16_t adcb2result = AdcbResultRegs.ADCRESULT0;
    adcb2_volts = adcb2result * 3.0 / 4095.0;
    // sjew: set the dac for scope output
    setDACA(adcb2_volts);
