...
// sample counters
uint32_t count = 10;
uint32_t light_count = 10;
uint32_t light_flag = 0; // 0 is light off
...
//!!!!!!!!!!!!!!!!!!!!!!  End of Block
    ...
    while(1)
    {
        if (UARTPrint == 1 ) {
            ...
            UART_printfLine(2,"Freq %.0f",maxpwrindex*10000.0/1024.0);
            UARTPrint = 0;
        }
//!!!!!!!!!!!!!!!!!!!!!!  Copy this block of code after your UARTPrint == 1 while loop as above
        if ( (pingFFT == 1) || (pongFFT == 1) ) {
            ...
            float range_lo = 1000;
            float power_lo = 100;
            float freq = maxpwrindex*10000.0/1024.0;
            int inrange = 0;

            if( freq < range_up && freq > range_lo && maxpwr > power_lo){
                inrange = 1;
            }
