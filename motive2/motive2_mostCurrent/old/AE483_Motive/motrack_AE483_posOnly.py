...
def unPack(data): #{{{1
    ...
    if (messageID == 7):
        ...
        while (i > 0):
            ...
            offset += 12
            trackableMarker.append([frameNumber,i,nUMarkers,ux,uy,uz]) # append frame , pt # , out # of pts, x, y, z
			i=i-1
    ...
    #return trackableState  # ONLY WANT NON-RIGID BODY PT DATA
	return trackableMarker
	
	# NEXT NEED TO CHANGE ___name____ !!
    ...
    while (True):
        ...
        if data:
            ...
            for i in range(nRigidBodies):
                ...
                pymsg[i] = struct.pack('ffffffff', mocap_state[i][0], mocap_state[i][1], mocap_state[i][2],\
				mocap_state[i][3],mocap_state[i][4], mocap_state[i][5], status[i], frame_counter)  # sending rbid instead of status
