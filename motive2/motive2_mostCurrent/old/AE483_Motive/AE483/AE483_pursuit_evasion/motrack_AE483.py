...
#drone_port = 3500
udp_port = 3500
address_list = ["192.168.1.117",#103 
				"192.168.1.117"]
    ...
    while (True):
        ...
        if data:
            ...
            for i in range(nRigidBodies):
                ...
                rbid[0],fn1,x1,y1,z1,qw1,qx1,qy1,qz1 = state[0] # rigid body 1
                rbid[1],fn2,x2,y2,z2,qw2,qx2,qy2,qz2 = state[1] # rigid body 1

                # Optitrack Sends Quaternions so convert data
                quat = Quaternion()
                quat.w1, quat.x1, quat.y1, quat.z1, quat.w2, quat.x2, quat.y2, quat.z2 = qw1, qx1, qy1, qz1, qw2, qx2, qy2, qz2
                ...
                mocap_state[0] = np.array([x1, y1, z1]) #(angles.phi1)*180/math.pi, (angles.theta1)*180/math.pi, (angles.psi1)*180/math.pi])
                mocap_state[1] = np.array([x2, y2, z2]) # (angles.phi2)*180/math.pi, (angles.theta2)*180/math.pi, (angles.psi2)*180/math.pi])
                ...
                threshold2 = 1e-9
                d = np.dot(mocap_state[0][0:2], mocap_state[1][0:2])
                if d < threshold2:
                    status[0] = 0
                    status[1] = 0
                else:
                ...
                if frame_counter>1:
                    d = np.dot(mocap_state[0][0:2]-prev_state[0][0:2],mocap_state[1][0:2]-prev_state[1][0:2])
                    if d < threshold2:
                        status[0] = 0
                        status[1] = 0
                    else:
                ...
                prev_state[0]=mocap_state[0]
                prev_state[1]=mocap_state[1]

                # Pack X,Z,YAW,STATUS(either 0 or RBID)
                #print "Packing rigid body number %d" % i
                pymsg[0] = struct.pack('ffffff', mocap_state[0][0], mocap_state[0][1], mocap_state[0][2],
                        mocap_state[1][0],mocap_state[1][1], mocap_state[1][2])  # sending rbid instead of status
            ...
            #print map(ord,pymsg_data)
                if i == 0:
                    print(pymsg[0])
                    udpsock.sendto(pymsg[0], ("192.168.1.117", udp_port))
                    if printnow == 1:
                        print("Sending to: %s" % (address_list[rbid[i]-1]))
                # if printnow == 1:
                    # print ("%.0f %.0f %.3f %.3f %.3f" % (rbid[i], status[i], mocap_state[1][0], mocap_state[1][1], mocap_state[1][2],
                                                        # mocap_state[2][0], mocap_state[2][1], mocap_state[2][2]))
