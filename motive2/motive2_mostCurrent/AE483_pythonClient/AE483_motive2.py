# Getting rigid body data from motive 2.0 for AE483
...
# address list the robots 
# 21 - Nagesh Eranki
address_id_dict = {21:"192.168.1.182"}
...
def __unpack_rigid_body(data, major, minor, rb_num):
    ...
    rigid_body.quat_val = quat
    angles = optiquat2euler(quat.vec())
    rigid_body.phi = angles.phi
    rigid_body.theta = angles.theta
    rigid_body.psi = angles.psi
            ...
            if message_id == 7 : #NAT_FRAMEOFDATA
                ...
                if rigid_body_count > 0:
                    if (frame_number % 25 == 0):
                            print("\n")
                            print("id     frame number     x    y    z    phi    theta    psi  ")
                for rigid_body in rigid_body_data:
                    x = rigid_body.pos[0]
                    y = rigid_body.pos[1]
                    z = rigid_body.pos[2]
                    quat = rigid_body.quat_val
                    theta = rigid_body.theta *180 / math.pi # need to convert to degrees for it to be conpatible with the C code
                    phi = rigid_body.phi *180 / math.pi # need to convert to degrees for it to be conpatible with the C code
                    psi = rigid_body.psi *180 / math.pi # need to convert to degrees for it to be conpatible with the C code
                    id_rb = rigid_body.id_num
                    udp_message = struct.pack('ffffffffffff', x,y,z,quat.x, quat.y, quat.z, quat.w, phi,theta,psi,id_rb,frame_number) #pack udp message
                    ...
                    try:
                        ...
                        pass
                    if (frame_number % 25 == 0):
                        print("%.0f    %.0f     %.3f    %.3f    %.3f     %.3f    %.3f      %.3f" % (id_rb, frame_number,x,y,z, phi,theta,psi))
